...
- (void)viewDidLoad {
    ...
    self.appearanceSettings = [LNEAppearanceSettings new];
    self.hb_appearanceSettings = [self appearanceSettings];


    self.preferences = [[HBPreferences alloc] initWithIdentifier: @"love.litten.lunepreferences"];


    self.enableSwitch = [UISwitch new];
    [[self enableSwitch] setOnTintColor:[UIColor colorWithRed: 0.60 green: 0.96 blue: 1.00 alpha: 1.00]];
    ...
    [[self iconView] setImage:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/LunePrefs.bundle/icon.png"]];
    ...
    [[self headerImageView] setImage:[UIImage imageWithContentsOfFile:@"/Library/PreferenceBundles/LunePrefs.bundle/Banner.png"]];
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/Lune.disabled"]) {
        [[self enableSwitch] setEnabled:NO];

        UIAlertController* alertController = [UIAlertController alertControllerWithTitle:@"Lune" message:@"Lune has detected that you have disabled it with iCleaner Pro, here are some quick actions you can perform" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)resetPrompt {
    ...
    UIAlertController* resetAlert = [UIAlertController alertControllerWithTitle:@"Lune" message:@"Do you really want to reset your preferences?" preferredStyle:UIAlertControllerStyleActionSheet];
...
- (void)resetPreferences {
    ...
    [[NSFileManager defaultManager] removeItemAtPath:@"/var/mobile/Library/Preferences/love.litten.lune.colorspreferences.plist" error:nil];
...
- (void)respring {
    ...
    [UIView animateWithDuration:1.0 delay:0.0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:@"/Library/MobileSubstrate/DynamicLibraries/shuffle.dylib"])
            ...
            [HBRespringController respringAndReturnTo:[NSURL URLWithString:@"prefs:root=Tweaks&path=Lune"]];
    }];
